...
#define LimitRequestFields 50 // Limits the number of HTTP request header fields that will be accepted from the client
#define LimitRequestFieldSize 4094 // Limits the size of the HTTP request header allowed from the client
#define LimitRequestLine 8190 // Limit the size in BYTES of the HTTP request line that will be accepted from the client
...
//    printf("DEBUG: index %s\n", index);
    ...
    char* filename = malloc( strlen(path) + strlen(index) + 1);
...
//    printf("DEBUG: index %s\n", index);
        ...
        free (filename);
        filename = malloc( strlen(path) + strlen(index) + 1 );
        strcpy(filename, path);
        strcat(filename, index);
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char* buffer;
    char *buffer_dest = abs_path;
    int buf_index = 0;
    
    buffer = calloc(LimitRequestLine + 1, 1);
    if(buffer == NULL)
    {
        error(500);
        return false;
    ...
    for (int i = 4; i < line_len - 11; i++)
    {
        if (buf_index > LimitRequestLine)
        {
        ...
        {
            memcpy(buffer_dest, buffer, strlen(buffer));
            buffer_dest[buf_index] = '\0';
